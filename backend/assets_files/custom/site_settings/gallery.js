...
$(document).ready(function() {
  ...
  var dell_item = function () {
    ...
    swal({
      ...
      function () {
        ...
        $.ajax({
          type: "post",
          url: "/site-settings/dell-picture.html",
          data: {
            'item_id': item_id,
            _csrf: csrfToken
          },
  ...
  var edit_item = function () {
    ...
    $('#sitesettingspictureform-image_id').val(item_id);
    ...
    $('#cropper-example-2-modal').modal('show').on('shown.bs.modal', function () {
      ...
      $('#sitesettingspictureform-image_id').val(null);
    });
  ...
  function showCoords(c) {
    $('#sitesettingspictureform-x').val(c.x);
    $('#sitesettingspictureform-y').val(c.y);
    $('#sitesettingspictureform-h').val(c.height);
    $('#sitesettingspictureform-w').val(c.width);
  }
  ...
  $('#picture-done-button').on('click',function(){
    ...
    $("#upload-picture-form").ajaxSubmit({
      url: '/site-settings/add-picture.html',
      type: 'post',
      ...
      success: function(json){
        ...
        if(json.error){
          ...
          $('#item_'+json.picture_id).replaceWith('<tr id="item_'+json.picture_id+'" sort-id="'+json.picture_id+'" ><td><span class="label label-info dd-h"><i class="fa fa-list"></i></span></td><td><a class="galery_img" data-lightbox="roadtrip" href="'+json.picture_src+'"><img alt="image" class="img-responsive" src="'+json.picture_thumb+'"></a></td><td>' +
            ...
            '</td><td><a id="text_'+json.picture_id+'" href="#" data-url="'+json.save_url+'"  data-block-id ="'+json.picture_id+'"   data-emptytext="Текст" data-mode="inline"  data-type="textarea"  data-pk="'+json.picture_id+'"  data-name="description" data-placement="right" data-placeholder="Описание"  class="editable editable-click item-settings myeditable save-item" data-original-title="" title="" style="background-color: rgba(0, 0, 0, 0);"></a></td>' +
            '<td><a href="#" class="edit-blog-item" data-picture-url="'+json.picture_src+'" data-item-id="'+json.picture_id+'">Изменить</a> |<a href="#" class="dell-blog-item" data-item-id="'+json.picture_id+'" >Удалить</a></td></tr>').
          on('click', '.dell-blog-item', dell_item);
          $('.save-item').editable();
